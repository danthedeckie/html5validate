...
class TestBrokenHTML(unittest.TestCase):
    ...
    def test_invalid_attr(self):
        with self.assertRaises(html5validate.exceptions.InvalidAttribute):
            validate(
                """<!doctype html><html><body><h1 class"NO">hi</h1></body></html> """
            )
    ...
    def test_bad_div(self):
        ...
        validate(
            """<!doctype html><html><body><a data-stuff="banana">hi</a></body></html>"""
        )
            ...
            with open(filename) as html:
                with self.subTest(f=filename):
                    with self.assertRaises(
                        html5validate.exceptions.ValidationException
                    ):
                        validate(html.read())
...
class TestHTML5LibParser(unittest.TestCase):
    def setUp(self):
        self.parser = html5lib.HTMLParser(
            html5lib.treebuilders.getTreeBuilder("dom"), strict=True
        )
            ...
            with open(filename) as html:
                with self.subTest(f=filename):
                    with self.assertRaises(
                        (
                            html5lib.html5parser.ParseError,
                            html5validate.exceptions.MisplacedElement,
                        )
                    ):
                        self.validate(html.read())
            ...
            with open(filename) as html:
                with self.subTest(f=filename):
                    with self.assertRaises(
                        (
                            html5lib.html5parser.ParseError,
                            html5validate.exceptions.MisplacedEndTag,
                        )
                    ):
                        self.validate(html.read())
            ...
            with open(filename) as html:
                with self.subTest(f=filename):
                    with self.assertRaises(
                        (
                            html5lib.html5parser.ParseError,
                            html5validate.exceptions.InvalidAttribute,
                        )
                    ):
                        self.validate(html.read())
